...
var namere= ($("#login_email").val() || "").trim();	

frappe.call({
    method: "frappe.api.is_face_active",
     args:{
		usr:namere
		},
    freeze:true, 
...
// start my function for face detection 
    ...
    var usernametrim = ($("#login_email").val() || "").trim();
    ...
    timer = setInterval( function() { 
            ...
            var newblob = dataURItoBlob(String(data));
			var loginDetails={  
			    flag : "login_page",  
			    username : usernametrim ,
			    image : data,
			    login_encoding_face : login_encoding_face_msg,
			    stop:"False"

			    // path:path_to_shape_predictor
			    };  

	    	// console.log(data);
            ws.send(JSON.stringify(loginDetails));
...
// end if for check is active 
}
// end of frappe call for user 
  }
    }
});
